...
Finder에서 실행한 경우에도 앱 내 업데이트가 Homebrew에 사용자 홈과 실행 경로를
전달하므로 터미널 환경을 별도로 설정할 필요가 없습니다.
...
- 앱 내 업데이트에서 `$HOME` 오류가 나면 최신 버전으로 업데이트한 뒤 다시
  시도하세요. 이전 버전에서는 터미널에서 Formula를 업데이트할 수 있습니다.
