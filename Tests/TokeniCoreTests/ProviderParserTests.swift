...
struct ProviderParserTests {
    ...
    @Test
    func codexDoesNotUseDelayedAccountBucketsForGrowth() {
        #expect(CodexUsageProvider().growthObservation(todayUsage: nil) == nil)
    }
    ...
    func defaultRegistryIncludesSupportedProviders() {
        ...
        #expect(ids == [.codex, .claude, .copilot, .cline, .antigravity, .grok])
    ...
    func claudeShowsConnectedWhenTodaySessionHasNoUsageYet() async throws {
        ...
        #expect(snapshot.connectionState == .localOnly)
