...
static void exercise_controls(void *context)
{
    ...
    SendMessageW(tokeni_dashboard_window, WM_COMMAND, 540, 0);
    assert(IsWindowVisible(tokeni_history_table));
    assert(!IsWindowVisible(tokeni_usage_list));
    assert(ListView_GetItemCount(tokeni_history_table) == 1);
    SendMessageW(tokeni_history_mode, BM_SETCHECK, BST_UNCHECKED, 0);
    SendMessageW(tokeni_dashboard_window, WM_COMMAND, 540, 0);
    assert(IsWindowVisible(tokeni_usage_list));
    SendMessageW(tokeni_theme_picker, CB_SETCURSEL, 2, 0);
    SendMessageW(tokeni_dashboard_window, WM_COMMAND, MAKEWPARAM(531, CBN_SELCHANGE), (LPARAM)tokeni_theme_picker);
    assert(tokeni_dark);
    SendMessageW(tokeni_language_picker, CB_SETCURSEL, 2, 0);
    SendMessageW(tokeni_dashboard_window, WM_COMMAND, MAKEWPARAM(530, CBN_SELCHANGE), (LPARAM)tokeni_language_picker);
    assert(tokeni_windows_dashboard_is_korean());
